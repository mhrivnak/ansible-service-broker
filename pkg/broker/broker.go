	...
	*/
	...
	if si != nil && uuid.Equal(si.ID, serviceInstance.ID) {
		if reflect.DeepEqual(si.Parameters, serviceInstance.Parameters) {
			alreadyInProgress, jobToken, err := a.isJobInProgress(serviceInstance, bundle.JobMethodProvision)
			if err != nil {
...
func (a AnsibleBroker) Deprovision(
	...
	alreadyInProgress, jobToken, err := a.isJobInProgress(&instance, bundle.JobMethodDeprovision)
	if err != nil {
...
func (a AnsibleBroker) isJobInProgress(instance *bundle.ServiceInstance,
	method bundle.JobMethod) (bool, string, error) {

	allJobs, err := a.dao.GetSvcInstJobsByState(instance.ID.String(), bundle.StateInProgress)
	log.Infof("All Jobs for instance: %v in state:  %v - \n%#v", instance.ID, bundle.StateInProgress, allJobs)
	if err != nil {
		return false, "", err
	}
...
func (a AnsibleBroker) Unbind(
	...
	jobInProgress, jobToken, err := a.isJobInProgress(&instance, bundle.JobMethodUnbind)
	if err != nil {
...
func (a AnsibleBroker) Update(instanceUUID uuid.UUID, req *UpdateRequest, async bool, userInfo UserInfo,
	...
	*/
	...
	////////////////////////////////////////////////////////////
	alreadyInProgress, jobToken, err := a.isJobInProgress(si, bundle.JobMethodUpdate)
	if err != nil {
